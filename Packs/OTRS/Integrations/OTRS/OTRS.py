        ...
        for article in articles:
            ...
            if attachments:
                for attachment in attachments:
                    ...
                    attachments_list.append({"path": attachment_file["FileID"], "name": file_name})
...
def demisto_entry_to_otrs_attachment(entry_list: list[Any]):
    ...
    for file in entry_list:
        ...
        otrs_attachment = Attachment.create_basic(  # Creating OTRS attachment object
            Filename=file_path["name"], Content=encoded_file, ContentType=content_type
        )
...
class OTRSClient:
    ...
    def search_ticket(
        ...
        article_create_time_newer_minutes: int | None = None,
...
def get_ticket_command(client: Client, args: dict[str, str]):
    ...
    if articles:
        ...
        for article in articles:
            ...
            if attachments:
                ...
                for attachment in attachments:
                    ...
                    current_attachment = {"Name": file_name, "Size": file_size, "ContentType": content_type}
        ...
        human_readable += tableToMarkdown(
            "Articles",
            human_readable_articles,
            headers=["ID", "From", "Subject", "Body", "CreateTime", "ContentType", "Attachment"],
            removeNull=True,
        )
...
def create_ticket_command(client: Client, args: dict[str, str]):
    ...
    return CommandResults(
        outputs=context, outputs_prefix="OTRS.Ticket", outputs_key_field="ID", readable_output=output, raw_response=context
    )
...
def close_ticket_command(client: Client, args: dict[str, str]):
    ...
    context = {"ID": ticket["TicketID"], "State": state, "Article": article_object}
...
def get_remote_data_command(client: Client, args: dict[str, str]):
    ...
    if args.get("lastUpdate"):
        last_update = round(datetime.strptime(args["lastUpdate"].split(".")[0], "%Y-%m-%dT%H:%M:%S").timestamp())
    else:
        last_update = 0
    retry_count = 3
    ...
    if parsed_args.entries:
        for entry in parsed_args.entries:
            ...
            if entry.get("file"):
                ...
                otrs_attachment = Attachment.create_basic(  # Creating OTRS attachment object
                    Filename=file_path["name"], Content=encoded_file, ContentType=entry["fileMetadata"]["type"]
                )
